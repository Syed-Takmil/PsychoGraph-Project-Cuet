...
export default function PatternMemoryGame() {
  ...
  useEffect(() => {
    if (gameState === 'complete') {
      const currentPath = '/activities/patternMemory'
      const roundsCompleted = round - 1

      // Local Persistence
      markCompleted(currentPath)
      saveActivityResult(currentPath, { roundsCompleted })
      ...
      fetch('http://localhost:5000/api/psychograph/save-results', {
        ...
        body: JSON.stringify({
          ...
          metrics: {
            roundsCompleted,
            maxSequenceLength: roundsCompleted + 1,
            completedAt: new Date().toISOString(),
          },
        }),
      }).catch((err) => console.error('Failed to dispatch Pattern Memory telemetry:', err))
    }
  }, [gameState, round, activeUserId])
  ...
  return (
    <div className="flex-1 flex items-center justify-center p-6 bg-gradient-to-br from-indigo-100 via-purple-50 to-pink-100 dark:from-gray-900 dark:via-purple-950 dark:to-gray-900">
      <div className="w-full max-w-lg bg-white/40 dark:bg-gray-900/60 backdrop-blur-2xl rounded-3xl shadow-2xl border border-white/50 dark:border-gray-700/50 p-8 text-center">
        <h2 className="text-xl font-semibold text-gray-700 dark:text-gray-200 mb-2">Activity 3: Pattern Memory</h2>
        ...
        {gameState === 'complete' && (
          <div className="mt-6">
            ...
            <div className="mt-4">
              <NextActivity currentPath="/activities/patternMemory" />
            </div>
          </div>
        )}
      </div>
