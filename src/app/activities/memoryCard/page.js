        ...
        if (newMatched.size === cards.length) {
          setTimeout(() => {
            if (currentLevel + 1 < LEVELS.length) {
              ...
              const currentPath = '/activities/memoryCard'
              const finalReport = generateReportAndMood(telemetry.current)
              setReport(finalReport)
              
              // Local Persistence
              markCompleted(currentPath)
              saveActivityResult(currentPath, finalReport)

              // Backend Persistence via Express Route
              fetch('http://localhost:5000/api/psychograph/save-results', {
                ...
                body: JSON.stringify({
                  ...
                  metrics: {
                    ...
                    totalTimeSec: finalReport.timeSec,
                    averageLatencyMs: finalReport.avgSpeed,
                    mistakes: finalReport.mistakes,
                    predictedMood: finalReport.mood.mood,
                    completedAt: new Date().toISOString()
                  }
                })
              }).catch(err => console.error('Failed to dispatch Memory Card telemetry:', err))
      ...
      <div className="w-full max-w-xl bg-gray-800/80 rounded-2xl p-6 border border-gray-700 shadow-xl text-center">
        ...
        {gameState === 'complete' && report && (
          <div className="text-left space-y-4">
            ...
            <div className="mt-3">
              <NextActivity currentPath="/activities/memoryCard" />
            </div>
          </div>
        )}
