...
const LEVELS = [
  { label: 'Easy', targetCount: 4, duration: 10, speed: { min: 0.4, max: 1.2 }, dangerAt: 0, shrinkAt: 18 },
  { label: 'Medium', targetCount: 5, duration: 10, speed: { min: 0.6, max: 2.0 }, dangerAt: 0, shrinkAt: 16 },
  { label: 'Hard', targetCount: 6, duration: 15, speed: { min: 1.0, max: 3.0 }, dangerAt: 0, shrinkAt: 14 },
]
...
export default function ClickAccuracyGame() {
  ...
  const finishGame = useCallback(async (results) => {
    stopAll()
    setGameState('complete')
    setLevelResults(results)

    const currentPath = '/activities/clickAccuracy'
    ...
    // Save locally
    saveActivityResult(currentPath, payload)

    // Dispatch Telemetry Payload strictly to Express Backend
    try {
      await fetch('http://localhost:5000/api/psychograph/save-results', {
        ...
        body: JSON.stringify({
          userId: activeUserId,
          gameName: 'clickAccuracy',
          metrics: payload,
        }),
      })
    } catch (err) {
      console.error('Failed to dispatch Click Accuracy telemetry:', err)
    }
  }, [stopAll, activeUserId])
        ...
        {gameState === 'start' && (
          <div className="bg-white/40 dark:bg-gray-900/60 backdrop-blur-2xl rounded-3xl shadow-2xl border border-white/50 dark:border-gray-700/50 p-10 text-center">
            <h1 className="text-3xl md:text-4xl font-bold text-gray-800 dark:text-gray-100 mb-4 tracking-tight">
              Activity 4: Click Accuracy Challenge
            </h1>
            ...
            <button
              onClick={handleStart}
              className="px-10 py-3.5 bg-gradient-to-r from-purple-600 to-pink-500 text-white font-semibold text-lg rounded-xl shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 transition-all cursor-pointer"
            >
              Start Challenge
            </button>
          </div>
          ...
          <>
            ...
            <div className="flex items-center justify-between mt-3">
              ...
              <button
                ...
                className="px-6 py-2 text-sm text-gray-400 dark:text-gray-500 hover:text-red-500 dark:hover:text-red-400 border border-gray-300 dark:border-gray-600 hover:border-red-300 dark:hover:border-red-700 rounded-xl transition-colors cursor-pointer"
              >
                Give Up
              </button>
            </div>
        ...
        {gameState === 'complete' && (
          <div className="bg-white/40 dark:bg-gray-900/60 backdrop-blur-2xl rounded-3xl shadow-2xl border border-white/50 dark:border-gray-700/50 p-10 text-center">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <button
                onClick={handleStart}
                className="px-8 py-3 bg-gradient-to-r from-purple-600 to-pink-500 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 transition-all cursor-pointer"
              >
                Play Again
              </button>
            </div>
            ...
            <div className="mt-6 flex items-center justify-center gap-4">
              <NextActivity currentPath="/activities/clickAccuracy" />
            </div>
          </div>
        )}
