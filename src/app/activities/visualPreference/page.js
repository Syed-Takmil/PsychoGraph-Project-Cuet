...
export default function VisualPreferenceTest() {
  ...
  // SUBMIT DATA TO STORAGE & EXPRESS BACKEND ON COMPLETION
  useEffect(() => {
    if (gameState === 'complete' && resultsSummary) {
      const currentPath = '/activities/visualPreference'
      ...
      fetch('http://localhost:5000/api/psychograph/save-results', {
        ...
        body: JSON.stringify({
          userId: session?.user?.id || 'guest_user',
          gameName: 'visualPreference',
          metrics: payload,
        }),
      }).catch((err) => console.error('Failed to dispatch Visual Preference telemetry:', err))
    }
  }, [gameState, resultsSummary, emotionTally, session])
        ...
        <div className="flex-[2] bg-white/40 dark:bg-gray-900/60 backdrop-blur-2xl rounded-3xl shadow-2xl p-8 border border-white/50 dark:border-gray-700/50 min-w-0">
          ...
          {gameState === 'start' && (
            <div className="text-center mt-10">
              ...
              <button
                onClick={beginTest}
                className="px-8 py-3 bg-gradient-to-r from-purple-600 to-pink-500 text-white font-semibold text-lg rounded-xl shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 transition-all cursor-pointer"
              >
                Start Visual Test
              </button>
            </div>
          ...
          {gameState === 'complete' && resultsSummary && (
            <div>
              ...
              <div className="mt-6 flex flex-col sm:flex-row items-center gap-4">
                ...
                <button
                  onClick={() => window.location.href = '/dashboard/psychograph'}
                  className="px-8 py-3 bg-white/60 dark:bg-gray-800/60 border border-purple-200 dark:border-purple-800 text-purple-700 dark:text-purple-300 font-semibold text-lg rounded-xl hover:bg-purple-50 transition-all cursor-pointer"
                >
                  View Full Psychograph Report 📊
                </button>
              </div>

              <div className="mt-4">
                <NextActivity currentPath="/activities/visualPreference" />
              </div>
            </div>
          )}
        </div>
