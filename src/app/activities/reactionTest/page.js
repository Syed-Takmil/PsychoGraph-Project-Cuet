...
export default function AdvancedReactionTest() {
  ...
  useEffect(() => {
    if (gameState === 'complete') {
      const currentPath = '/activities/reactionTest'
      ...
      const variance = reactionTimes.length > 1
        ? reactionTimes.reduce((sum, val) => sum + Math.pow(val - avg, 2), 0) / reactionTimes.length
        : 0
      ...
      // Save local activity result
      saveActivityResult(currentPath, payload)

      // Dispatch Telemetry Payload to Express Backend
      ...
      }).catch((err) => console.error('Failed to dispatch Reaction Test telemetry:', err))
    }
  }, [gameState, falseStarts, reactionTimes, session])
  ...
  return (
    <div className="flex-1 flex items-center justify-center p-4 bg-gradient-to-br from-indigo-100 via-purple-50 to-pink-100 dark:from-gray-900 dark:via-purple-950 dark:to-gray-900">
      <div className="w-full max-w-xl bg-white/40 dark:bg-gray-900/60 backdrop-blur-2xl rounded-3xl shadow-2xl p-8 md:p-10 border border-white/50 dark:border-gray-700/50 text-center">
        ...
        <div className="mb-5">
          <h2 className="text-xl font-semibold text-gray-700 dark:text-gray-200">Activity 5: Reaction Time Test</h2>
        ...
        {gameState === 'start' && (
          <div>
            ...
            <button
              ...
              className="px-10 py-3 bg-gradient-to-r from-purple-600 to-pink-500 text-white font-semibold text-lg rounded-xl shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 transition-all w-full cursor-pointer"
            >
              Begin Test Sequence
            </button>
          </div>
            ...
            {gameState === 'early' && (
              <div>
                ...
                <button
                  ...
                  className="mt-4 px-4 py-2 bg-white/20 hover:bg-white/30 rounded-lg text-white text-sm font-semibold transition-colors cursor-pointer"
                >
                  Resume Attempt
                </button>
              </div>
            ...
            {gameState === 'result' && (
              <div>
                ...
                <button
                  ...
                  className="mt-4 px-4 py-2 bg-white/20 dark:bg-white/10 hover:bg-white/30 dark:hover:bg-white/20 rounded-lg text-gray-700 dark:text-gray-200 text-sm font-semibold transition-colors cursor-pointer"
                >
        ...
        {gameState === 'complete' && (
          <div>
            ...
            <div className="mt-4">
              <NextActivity currentPath="/activities/reactionTest" />
            </div>
          </div>
        )}
